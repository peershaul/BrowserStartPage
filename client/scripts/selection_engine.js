...
class Selectable {
	...
	static unfocus = (primaries) => {
		for (let i = 0; i < primaries.length; i++) {
			if (primaries[i].elem.classList.contains('focus')) {
				...
				if (primaries[i].elem.nodeName == 'INPUT') primaries[i].elem.blur();
			}
	...
	focus = (primaries) => {
		...
		if (this !== focused) Selectable.unfocus(primaries);
		this.elem.classList.add('focus');
		if (this.elem.nodeName === 'INPUT') this.elem.focus();
	};
}
...
function genHirarchy(raw_list) {
	...
	for (let i = 0; i < raw_list.length; i++) {
		if (Number.isInteger(raw_list[i][1])) primaries.push(new PrimarySelectable(raw_list[i][0], raw_list[i][1]));
		else
...
function find_from_primaries(prime, primaries) {
	for (let i = 0; i < primaries.length; i++) if (prime == primaries[i]) return i;

	return NaN;
}

function find_from_primary(secondary, prime) {
	for (let i = 0; i < prime.secondaries.length; i++) if (secondary == prime.secondaries[i]) return i;
	...
	if (is_secondary) {
		...
		if (index == 0) {
			if (primaries[primaries.length - 1].secondaries.length > 0)
				primaries[primaries.length - 1].secondaries[
					primaries[primaries.length - 1].secondaries.length - 1
				].focus(primaries);
